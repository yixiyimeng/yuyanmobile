...
(function(window, document, undefined) {
	...
	function swipeDelete(element, options) {
		...
		//取消new 关键字
		return (_self instanceof swipeDelete) ? _self.init(_self.swipeElement, options) : new swipeDelete(_self.swipeElement,
			options);
	...
	swipeDelete.prototype = {
		...
		swipeEvent: function(element, options) {
			...
			ele.addEventListener('touchstart', function(event) {
				...
				startX = (ele.style.WebkitTransform.replace(/translateX\(/g, "").replace(/(px|rem|%)\)/g, "")) * 1 || (ele.style
					.transform.replace(/translateX\(/g, "").replace(/(px|rem|%)\)/g, "")) * 1 || 0;
				...
				document.addEventListener('touchend', function(event) {
					...
					if (timeDiff < 300 && (touchesDiff < -10 && direction === 'left' || touchesDiff > 10 && direction ===
							'right') ||
						timeDiff > 300 && (touchesDiff < -parseInt(distance / 3) && direction === 'left' || touchesDiff > parseInt(
							distance / 3) && direction === 'right')
					) {
						if (action == 'close') {
